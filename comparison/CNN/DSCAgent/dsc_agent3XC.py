...
class DSCAgent(base_agent.BaseAgent):
	...
	def step(self, obs):
		...
		if obs.last():
			self.Game_Loop+=1
			if self.Game_Loop%UPDATE_RATE==0 and os.path.isfile(QA.Main_Directory+'Net/NetDSC/Conv-net.pkl'):
				self.Q.eval_net.Conv.load_state_dict(torch.load(QA.Main_Directory+'Net/NetDSC/Conv-net.pkl'))
				self.Q.target_net.Conv.load_state_dict(torch.load(QA.Main_Directory+'Net/NetDSC/Conv-net.pkl'))
