...
class MSCAgent(base_agent.BaseAgent):
	""" 
	MSC Agent 1 step edition(三倍速) Used in self-play,cannot learn,update every few game loops
	...
	def step(self, obs):
		...
		if obs.last():
			...
			if self.Game_Loop%UPDATE_RATE==0 and os.path.isfile(QA.Main_Directory+'Net/NetDQN/r-net.pkl'):
				self.Q.eval_net.rNet.load_state_dict(torch.load(QA.Main_Directory+'Net/NetDQN/r-net.pkl'))
				self.Q.target_net.rNet.load_state_dict(torch.load(QA.Main_Directory+'Net/NetDQN/r-net.pkl'))
			if self.Game_Loop%UPDATE_RATE==0 and os.path.isfile(QA.Main_Directory+'Net/NetDQN/t-net.pkl'):
				self.Q.eval_net.tNet.load_state_dict(torch.load(QA.Main_Directory+'Net/NetDQN/t-net.pkl'))
				self.Q.target_net.tNet.load_state_dict(torch.load(QA.Main_Directory+'Net/NetDQN/t-net.pkl'))
